...
function renderHistoryItem(item) {
  ...
  if (item.note) {
    ...
    note.textContent = item.kind === 'reminder' ? item.note : `Note: ${item.note}`;
...
export function renderRenterDrawer({
  ...
  const monthStatus = monthSummary?.status || 'unknown';
  statusLine.innerHTML = `<span class="renter-status-badge status-${monthStatus}">${formatStatusLabel(monthStatus)}</span>`;
  ...
  const chargesTotal = Number(monthSummary?.chargesTotal || 0);
  const paymentsTotal = Number(monthSummary?.paymentsTotal || 0);
  const remaining = Number(monthSummary?.remaining || 0);

  const statusText = document.createElement('p');
  statusText.className = 'month-status-row';
  statusText.innerHTML = `<strong>Status:</strong> <span class="renter-status-badge status-${monthStatus}">${formatStatusLabel(monthStatus)}</span>`;

  const chargesText = document.createElement('p');
  chargesText.innerHTML = `<strong>Charges this month:</strong> ${formatMoney(chargesTotal)}`;

  const paymentsText = document.createElement('p');
  paymentsText.innerHTML = `<strong>Payments this month:</strong> ${formatMoney(paymentsTotal)}`;
  ...
  upcomingText.className = 'upcoming-charge-text';
  if (monthSummary?.upcomingCharge) {
    upcomingText.innerHTML = `<strong>Upcoming monthly charge on:</strong> ${formatTimestamp(monthSummary?.dueDate)}`;
  }
  ...
  reminderMessage.className = 'notes-message';
  ...
  markReminderButton.addEventListener('click', async () => {
    markReminderButton.disabled = true;
    reminderMessage.textContent = '';
    reminderMessage.className = 'notes-message';

    try {
      await onMarkReminderSent(renter.id);
      reminderMessage.textContent = 'Reminder marked sent.';
      reminderMessage.classList.add('success');
      showToastMessage('Reminder marked sent.');
    } catch (error) {
      reminderMessage.textContent = error?.message || 'Could not mark reminder as sent.';
      reminderMessage.classList.add('error');
    } finally {
      markReminderButton.disabled = false;
    }
  });
  ...
  logPanel.append(logTitle, amountField, methodField, otherMethodField, noteField, markReminderButton, reminderMessage, paymentError, savePaymentButton);

  monthPanel.append(monthTitle, monthLabel, statusText, chargesText, paymentsText, remainingText);
  if (monthSummary?.upcomingCharge) monthPanel.append(upcomingText);
  monthPanel.append(paymentsList, notesPreviewWrap, logPanel);
